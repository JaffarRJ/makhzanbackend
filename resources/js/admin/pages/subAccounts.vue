        ...
        <div class="content">
            <div class="container-fluid">
                ...
                <div class="_1adminOverveiw_table_recent _box_shadow _border_radious _mar_b30 _p20">
                    <p class="_title0"><Button @click="addModal=true"><Icon type="md-add" /> Add Sub Account</Button></p>
                ...
                <Modal v-model="addModal" title="Add Account" :mask-closable="false" :closable="false">
                    <div class="space">
                        <Input type="text" v-model="data.name" placeholder="Sub Account Name" autocomplete="off" />
                    </div>
                    <div class="space">
                        <textarea type="text" class="form-control" v-model="data.detail" placeholder="Sub Account Detail" autocomplete="off" />
                    </div>
                    <div slot="footer">
                ...
                <Modal v-model="editModal" title="Edit Role" :mask-closable="false" :closable="false">
                    ...
                    <div class="space">
                        <textarea type="text" class="form-control" v-model="editData.detail" placeholder="Sub Account Detail" autocomplete="off" />
                    </div>
                    <div slot="footer">
    ...
    export default {
        ...
        methods : {
            async addItem(){
                ...
                const res = await this.callApi('post', 'api/sub_account/store', this.data)
                if(res.status===200){
            ...
            async editItem(){
                ...
                const res = await this.callApi('post', 'api/sub_account/update', this.editData)
                if(res.status===200){
            ...
            async deleteItemBtn(){
                this.isDeleing = true;
                const res = await this.callApi('post', 'api/sub_account/delete', this.deleteItem)
                if(res.status===200){
            ...
            async statusItemBtn(){
                ...
                const res = await this.callApi('post', 'api/sub_account/updateIsActive', setIsActive)
                if(res.status===200){
                ...
                const res = await this.callApi('post', 'api/sub_account/updateIsShow', setIsShow)
                if(res.status===200){
                    // this.setData.splice(this.statusItem,1)
                    window.location.reload();
