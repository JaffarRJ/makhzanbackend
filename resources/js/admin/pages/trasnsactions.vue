        ...
        <div class="content">
            <div class="container-fluid">
                ...
                <div class="_1adminOverveiw_table_recent _box_shadow _border_radious _mar_b30 _p20">
                    <p class="_title0"><Button @click="addModal=true"><Icon type="md-add" /> Add Transaction</Button></p>
                ...
                <!-- adding modal -->
                <Modal v-model="addModal" title="Add Transaction" :mask-closable="false" :closable="false">
                    <div class="space">
                        <Input type="text" v-model="data.name" placeholder="Name" autocomplete="off" />
                    </div>
                    <div slot="footer">
                        <Button type="default" @click="addModal=false">Close</Button>
                        <Button type="primary" @click="addAdmin" :disabled="isAdding" :loading="isAdding">{{isAdding ? 'Adding..' : 'Add Transaction'}}</Button>
                    </div>

                </Modal>
                <!-- editing modal -->
                <Modal v-model="editModal" title="Edit Transaction" :mask-closable="false" :closable="false">
                    <div class="space">
                        <Input type="text" v-model="editData.name" placeholder="Name"  />
                    </div>
                    <div slot="footer">
                        <Button type="default" @click="editModal=false">Close</Button>
                        <Button type="primary" @click="editAdmin" :disabled="isAdding" :loading="isAdding">{{isAdding ? 'Editing..' : 'Edit Transaction'}}</Button>
                    </div>
                </Modal>
                <!-- delete alert modal -->
    ...
    export default {
        ...
        methods : {
            async addAdmin(){
                ...
                const res = await this.callApi('post', 'api/transaction/store', this.data)
            ...
            async editAdmin(){
                ...
                const res = await this.callApi('post', 'api/transaction/update', this.editData)
                if(res.status===200){
            ...
            async deleteItemBtn(){
                this.isDeleing = true;
                const res = await this.callApi('post', 'api/transaction/delete', this.deleteItem)
                if(res.status===200){
            ...
            async statusItemBtn(){
                ...
                const res = await this.callApi('post', 'api/transaction/updateIsActive', setIsActive)
                if(res.status===200){
                ...
                const res = await this.callApi('post', 'api/transaction/updateIsShow', setIsShow)
                if(res.status===200){
                    // this.setData.splice(this.statusItem,1)
                    window.location.reload();
